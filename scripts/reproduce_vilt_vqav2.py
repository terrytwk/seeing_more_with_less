...
def run_filter(images, outfolder, condition, batch_size, filter_workers, python_exe):
...
def main():
    ...
    parser.add_argument("--filter_workers", type=int, default=0, help="Parallel filter workers. 0 or 1 preserves serial filtering.")
    ...
    if not args.no_generate_sampling_maps:
        ...
        timings["sampling_maps_seconds"] = time.perf_counter() - stage_start
    ...
    for condition in ["variable", "uniform"]:
        ...
        if args.skip_filter:
            ...
            stage_start = time.perf_counter()
            image_roots[condition] = run_filter(images, outfolder, condition, args.batch_size, args.filter_workers, args.python)
            timings[f"{condition}_filter_seconds"] = time.perf_counter() - stage_start
    ...
    for condition, image_root in image_roots.items():
        ...
        eval_seconds = time.perf_counter() - stage_start
        timings[f"{condition}_eval_seconds"] = eval_seconds
    ...
    aggregate["timings_seconds"] = timings
